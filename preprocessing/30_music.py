...
f = open('30_data.csv','r')
f2 = open('30_data_more.csv','w',newline='')
rd = csv.reader(f)
data = list(rd)
wr = csv.writer(f2)
idx = 0
...
for j in range(300):
    ...
    for i in range(3):
        ...
        if i == 0:
            ...
            input = np.append((title + "_" + str(i * 29) + "_" + str((i+1) * 29)).replace('.mp3', '') + '.mp3', data[j][1:])
            # print((title + "_" + str(i * 29) + "_" + str((i+1) * 29)).replace('.mp3', '') + '.mp3', len(data[j][1:]))
            wr.writerow(input)